...
class RouterPrefixTester:
    ...
    def test_voice_router_endpoints(self):
        """Test voice router moved to /api/voice"""
        ...
        if success:
            voices = response.get('voices', []) if isinstance(response, dict) else response
            print(f"   🎵 Voice service response: {voices}")
        
        return success

    def test_admin_assistant_router(self):
        """Test admin assistant router moved to /api/admin-assistant"""
        ...
        if success:
            if isinstance(response, dict):
                print(f"   🤖 Admin Assistant Response keys: {list(response.keys())}")
            else:
                print(f"   🤖 Admin Assistant Response: {str(response)[:100]}...")
...
def main():
    print("🚀 Starting Router Prefix Smoke Tests")
    print("Testing /api prefix compliance and router updates")
    print("=" * 60)
    ...
    # Final results
    print("\n" + "="*60)
    print("📊 ROUTER PREFIX SMOKE TEST RESULTS")
    print("="*60)
    ...
    if failed_tests:
        ...
        print("\n🎉 ALL ROUTER PREFIX TESTS PASSED!")
