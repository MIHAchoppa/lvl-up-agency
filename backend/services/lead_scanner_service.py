...
class LeadScannerService:
    ...
    def __init__(self):
        self.platforms = {
            ...
            "tiktok": {
                ...
                "patterns": {"username": r"@([a-zA-Z0-9._]+)", "followers": r"(\d+(?:\.\d+)?[KkMm]?)\s+Followers"},
            },
            "youtube": {
                "search_url": "https://www.youtube.com/results?search_query={keyword}",
                "patterns": {"username": r"@([a-zA-Z0-9._]+)", "subscribers": r"(\d+(?:\.\d+)?[KkMm]?)\s+subscribers"},
            },
        }

    async def scan_for_leads(
        self, platforms: List[str], keywords: List[str], min_followers: int = 1000, max_results: int = 50
    ) -> Dict[str, Any]:
        """
        ...
        results = {
            ...
            "keywords": keywords,
            "filters": {"min_followers": min_followers, "platforms": platforms},
        }
        ...
        for platform in valid_platforms:
            try:
                platform_leads = await self._scan_platform(platform.lower(), keywords, min_followers, max_results)

                results["by_platform"][platform] = {"found": len(platform_leads), "status": "success"}
                ...
                results["by_platform"][platform] = {"found": 0, "status": "error", "error": str(e)}

        return results

    async def _scan_platform(
        self, platform: str, keywords: List[str], min_followers: int, max_results: int
    ) -> List[Dict[str, Any]]:
        ...
        for keyword in keywords[:3]:  # Limit to first 3 keywords
            sample_leads = self._generate_sample_leads(
                platform, keyword, min_followers, min(max_results // len(keywords), 20)
            )
    ...
    def _generate_sample_leads(
        self, platform: str, keyword: str, min_followers: int, count: int
    ) -> List[Dict[str, Any]]:
        """
